...
angular.module('tm.pagination', []).directive('tmPagination',[function(){
    return {
        ...
        template: '<div class="page-list">' +
            ...
            '跳转至<input type="text" name="jumpPageNum" ng-model="jumpPageNum" ng-paste="$event.preventDefault()"  ng-keyup="jumpPageKeyUp($event,jumpPageNum)"   uib-tooltip="必须输入正整数！" />' +
            '</div>' +
        ...
        link: function(scope, element, attrs) {
            ...
            scope.jumpPageKeyUp = function(e,value) {
                //console.log('jumpPageNum---前',scope.jumpPageNum);
               
                var keycode = window.event ? e.keyCode :e.which;

                if (value.length == 1) { // 第一个字符只能是1-9，
                ...
                // 如果大于最大值，去掉当前输入的值
                if (parseInt(value, 10) > conf.numberOfPages) {
                    scope.jumpPageNum = value.substring(0, value.length - 1);
                } else { // 否则直接赋值 
                    scope.jumpPageNum = value;
                }

                //console.log('jumpPageNum---后',scope.jumpPageNum);
            ...
            // scope.$watch('jumpPageNum', function (val) {
            //
            // })
