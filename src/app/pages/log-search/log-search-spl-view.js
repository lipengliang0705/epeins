(function() {
    ...
    angular
        ...
        .directive('logSearchSplView', logSearchSplView)
    ...
    function logSearchSplViewService($resource, $http, $filter, DateUtils, DataStore, EsService, toastr, ResultHandler, NgTableParams) {
        ...
        function execSpl(body, callback) {
            EsService.logSearchForSpl().post({spl: body}, function (data) {
                EsService.logSearchForSql().post({sql: data.sql}, callback, function(data) {toastr.pop('error', "错误", "查询时出错"+ JSON.stringify(data))})
            });
        }

        function execSql(sql, callback) {
                EsService.logSearchForSql().post({sql: sql}, callback, function(data) {toastr.pop('error', "错误", "sql 查询出错"+ JSON.stringify(data))})
        }
        ...
        function logSearchSplView() {
        return {
            restrict: 'EA',
            controller: 'SplViewCtrl',
            controllerAs: 'vm',
            bindToController: true,
            templateUrl: 'app/pages/log-search/log-search-spl-view.html',
            scope: {
                expression: '=',
                model: '='
            },
