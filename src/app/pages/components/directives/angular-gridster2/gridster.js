...
(function () {
  ...
  angular.module('angular-gridster2')
    .service('GridsterUtils', function () {
      ...
      //add by huiming.mu for resize problem in parent-child mode
      // ***** start ***** 
      function checkGridClassForResize(gridster, e) {
        if (gridster.$options.resizable.gridClass) {
          var gridElem = $(e.target).closest("gridster")[0];
          return !gridElem.classList.contains(gridster.$options.resizable.gridClass)
        } else {
          return false;
        }
      }
      ...
      return {
        ...
        checkGridClassForResize: checkGridClassForResize    //add by huiming.mu for resize problem in parent-child mode
      }
    });
})();
  ...
  function GridsterResizable(GridsterPush, GridsterScroll, GridsterUtils, GridsterPushResize) {
    return function (gridsterItem, gridster) {
      ...
      vm.dragStart = function (e) {
        ...
        //add by huiming.mu for resize problem in parent-child mode
        // ***** start ***** 
        if (GridsterUtils.checkGridClassForResize(vm.gridster, e)){
          return;
        }
        // ***** end *****
...
(function () {
  ...
  function GridsterController($element, GridsterConfig, GridsterUtils, $log, $scope, GridsterEmptyCell, GridsterCompact) {
    ...
    vm.$options.resizable.gridClass = undefined;    //add by huiming.mu for resize problem in parent-child mode
    ...
    vm.calculateLayout = function calculateLayout() {
      ...
      //   vm.curRowHeight = Math.floor((vm.curHeight + vm.$options.margin) / vm.rows);
      // }
      /********  let no offset when in fit mode  ********/
      if (vm.$options.outerMargin) {
